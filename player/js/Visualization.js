...
function audioSetup(audio, container, THREE, $) {
    ...
    let options = {
        ...
        get Count() {
            return Math.round((this._HighLimit - this._LowLimit) * this._count)
        },
        get LowLimit() {
            return Math.round(this._LowLimit * this._count);
        },
        get HighLimit() {
            return Math.round(this._HighLimit * this._count);
        }
    }
    ...
    audio.onplay = () => {
        cancelatioToken = process(analizer, graficalContext, freqData, options);
    }
...
function process(analizer, graficalContext, freqData, options) {
    ...
    graficalContext.spotLight.color = {
        r: freqData[60] / 255,
        g: freqData[60] / 255,
        b: freqData[60] / 255,
    }
